...
const hasVehicleLocation = computed(
...
function initMap() {
  ...
  const center = hasVehicleLocation.value
    ...
    : (deviceLocation.value ? [deviceLocation.value.lat, deviceLocation.value.lng] : [0, 0])
  ...
  navigator.geolocation.getCurrentPosition(
    (pos) => {
      ...
      if (map) {
        ...
        // Show real location instead of a fixed default
        if (hasVehicleLocation.value) {
